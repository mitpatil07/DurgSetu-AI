    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-indigo-900 to-black text-white relative overflow-hidden">
      ...
      {showUpload && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gradient-to-br from-purple-900/95 to-indigo-900/95 rounded-2xl p-8 max-w-2xl w-full border border-purple-500/30 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setShowUpload(false)} className="text-white hover:text-red-400 text-2xl">&times;</button>
            </div>
      ...
      {selectedFortDetails && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-gradient-to-br from-purple-900/95 to-indigo-900/95 rounded-2xl p-8 max-w-6xl w-full border border-purple-500/30 my-8">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-3xl font-bold text-white">{selectedFortDetails.name}</h2>
              <button onClick={() => setSelectedFortDetails(null)} className="text-white hover:text-red-400">
                <X className="w-8 h-8" />
              </button>
            </div>

            {selectedFortDetails.detailedAnalysis ? (
              <div className="space-y-6">
                {/* Annotated Image */}
                <div className="bg-black/40 rounded-xl p-4">
                  ...
                  <img 
                    src={selectedFortDetails.detailedAnalysis.annotated_image_url} 
                    alt="Annotated Analysis"
                    className="w-full rounded-lg border-2 border-purple-500/30"
                  />
                  <p className="text-purple-300 text-sm mt-2">
                    Red boxes indicate detected structural changes. Analysis performed on {new Date(selectedFortDetails.analysisDate).toLocaleString()}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-black/40 rounded-xl p-4">
                    ...
                    <img 
                      src={selectedFortDetails.detailedAnalysis.previous_image_url} 
                      alt="Previous"
                  ...
                  <div className="bg-black/40 rounded-xl p-4">
                    ...
                    <img 
                      src={selectedFortDetails.detailedAnalysis.current_image_url} 
                      alt="Current"
                      className="w-full rounded-lg border border-purple-500/30"
                    />
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-gradient-to-br from-red-500/20 to-pink-500/20 rounded-xl p-4 border border-red-400/30">
                    <div className="text-3xl font-bold text-red-300">{selectedFortDetails.changesDetected}</div>
                  ...
                  <div className="bg-gradient-to-br from-orange-500/20 to-yellow-500/20 rounded-xl p-4 border border-orange-400/30">
                    <div className="text-3xl font-bold text-orange-300">{selectedFortDetails.riskScore}/10</div>
                  ...
                  <div className="bg-gradient-to-br from-blue-500/20 to-cyan-500/20 rounded-xl p-4 border border-blue-400/30">
                    <div className="text-3xl font-bold text-blue-300">{selectedFortDetails.structuralHealth}%</div>
                  ...
                  <div className="bg-gradient-to-br from-purple-500/20 to-pink-500/20 rounded-xl p-4 border border-purple-400/30">
                    <div className="text-3xl font-bold text-purple-300">{selectedFortDetails.totalArea.toLocaleString()}</div>
                ...
                <div className={`rounded-xl p-6 border-2 ${
                  selectedFortDetails.riskLevel === 'HIGH' || selectedFortDetails.riskLevel === 'CRITICAL' 
                    ? 'bg-red-500/20 border-red-400/50' 
                    : selectedFortDetails.riskLevel === 'MEDIUM'
                    ? 'bg-yellow-500/20 border-yellow-400/50'
                    : 'bg-green-500/20 border-green-400/50'
                }`}>
                  <h3 className="text-2xl font-bold mb-2">Risk Assessment: {selectedFortDetails.riskLevel}</h3>
                  <p className="text-lg mb-4">{selectedFortDetails.prediction}</p>
                  
                  {selectedFortDetails.detailedAnalysis.risk_assessment?.factors && (
                    <div className="mb-4">
                      <h4 className="font-bold mb-2">Risk Factors:</h4>
                      <ul className="list-disc list-inside space-y-1">
                        {selectedFortDetails.detailedAnalysis.risk_assessment.factors.map((factor, i) => (
                          <li key={i}>{factor}</li>
                        ))}
                      </ul>
                    </div>
                ...
                {/* Recommendations */}
                {selectedFortDetails.recommendations.length > 0 && (
                  <div className="bg-blue-500/20 border border-blue-400/30 rounded-xl p-6">
                    <h3 className="text-xl font-bold text-blue-300 mb-4">Recommendations</h3>
                    <ul className="space-y-2">
                      {selectedFortDetails.recommendations.map((rec, i) => (
                ...
                {/* Detection Details */}
                {selectedFortDetails.detailedAnalysis.analysis_results?.detections && (
                  <div className="bg-black/40 rounded-xl p-6">
                    <h3 className="text-xl font-bold text-purple-300 mb-4 flex items-center gap-2">
                      <MapPin className="w-6 h-6" />
                      Detected Changes ({selectedFortDetails.detailedAnalysis.analysis_results.detections.length})
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {selectedFortDetails.detailedAnalysis.analysis_results.detections.map((detection, i) => (
      ...
      <div className="container mx-auto px-6 py-8 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {fortsData.length === 0 ? (
            ...
            fortsData.map((fort) => (
              <div key={fort.id} className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-md rounded-2xl p-6 border border-white/20 hover:border-purple-400/50 transition-all">
                <div className="flex items-center justify-between mb-4">
                  <div>
                ...
                <div className="space-y-3">
                  ...
                  <button
                    onClick={() => setSelectedFortDetails(fort)}
