    ...
    return (
        <div className="min-h-screen bg-[#F8F9FB]">
                ...
                <div className="max-w-5xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2.5">
                            ...
                            <span className="font-black text-slate-800 text-sm hidden sm:inline tracking-tight">DurgSetu</span>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        <button onClick={() => navigate('/report')}
                            className="flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-bold text-sm rounded-xl transition-all shadow-md shadow-orange-200 active:scale-95 cursor-pointer">
                            <Plus className="w-4 h-4" />
                            <span className="hidden sm:inline">Submit Report</span>
                        </button>

                        <div className="hidden sm:flex items-center gap-2 bg-orange-50 border border-orange-100 rounded-xl px-3 py-1.5">
                            <div className="w-6 h-6 bg-gradient-to-br from-orange-400 to-orange-600 rounded-lg flex items-center justify-center shadow-sm">
                                <span className="text-white font-black text-xs">{initial}</span>
                            </div>
                            <span className="text-sm font-bold text-orange-800">{username}</span>
                        </div>

                        <button onClick={() => { localStorage.clear(); navigate('/login'); }}
                            className="flex items-center gap-1.5 text-sm font-semibold text-slate-400 hover:text-red-600 px-3 py-2 rounded-xl hover:bg-red-50 transition-all cursor-pointer">
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-6">
                ...
                {/* ── Hero greeting ── */}
                <div className="bg-gradient-to-br from-orange-500 via-orange-600 to-red-600 rounded-3xl p-7 text-white relative overflow-hidden shadow-xl shadow-orange-200">
                    <div className="absolute top-0 right-0 -mr-10 -mt-10 w-48 h-48 rounded-full bg-white/10 blur-2xl pointer-events-none" />
                    <div className="absolute bottom-0 left-0 -ml-10 -mb-10 w-56 h-56 rounded-full bg-orange-400/20 blur-3xl pointer-events-none" />
                    <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <div>
                            <p className="text-orange-200 text-xs font-bold uppercase tracking-widest mb-1">Welcome back</p>
                            <h1 className="text-2xl sm:text-3xl font-black tracking-tight">Hey, {username} 👋</h1>
                            <p className="text-orange-100 text-sm mt-1.5 font-medium">
                                You've submitted <span className="text-white font-black">{reports.length}</span> report{reports.length !== 1 ? 's' : ''} to protect Maharashtra's forts.
                            </p>
                        </div>
                        <div className="flex items-center gap-2 bg-white/15 backdrop-blur-sm border border-white/20 rounded-2xl px-4 py-3 flex-shrink-0">
                            <Star className="w-5 h-5 text-yellow-300 fill-yellow-300" />
                            <div>
                                <p className="text-xs font-bold text-orange-200">Heritage Guardian</p>
                                <p className="text-sm font-black text-white">Active Reporter</p>
                            </div>
                        </div>
                    </div>
                </div>
