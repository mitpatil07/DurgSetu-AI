            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
                    {[
                        ...
                        <div key={label} className="bg-white rounded-xl border border-slate-100 p-3 sm:p-4 flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 overflow-hidden">
                            <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${accent}`}>
                                <Icon className="w-4 h-4 sm:w-5 sm:h-5" />
                            </div>
                            <div className="min-w-0 w-full">
                                <p className="text-lg sm:text-2xl font-extrabold text-slate-900 leading-none">{value}</p>
                                <p className="text-[10px] sm:text-xs text-slate-400 font-medium mt-1 pr-1 truncate">{label}</p>
                            </div>
                        </div>
                    ))}
                </div>
                                        ...
                                        <div key={r.id} className="p-5 space-y-4">
                                            <div className="flex items-start justify-between">
                                                <div className="min-w-0">
                                                    <p className="font-extrabold text-slate-900 text-sm truncate">{r.fort_name}</p>
                                                    <p className="text-[10px] text-slate-400 flex items-center gap-1 mt-0.5"><MapPin className="w-3 h-3 text-orange-400" /> {r.location}</p>
                                                </div>
                                                <Badge status={r.status} />
                                            </div>
                                            <div className="grid grid-cols-2 gap-4 py-2 border-y border-slate-50">
                                                <div>
                                                    <p className="text-[9px] font-bold text-slate-400 uppercase mb-0.5">Reporter</p>
                                                    <p className="text-xs font-bold text-slate-700 truncate">{r.user_name || '—'}</p>
                                                </div>
                                                <div>
                                                    <p className="text-[9px] font-bold text-slate-400 uppercase mb-0.5">Severity</p>
                                                    <p className="text-xs font-black text-slate-800">{r.severity}</p>
                                                </div>
                                                <div className="col-span-2">
                                                    <p className="text-[9px] font-bold text-slate-400 uppercase mb-0.5">Damage Type</p>
                                                    <p className="text-xs font-bold text-slate-700">{r.damage_type}</p>
                                                </div>
                                            </div>
                                            <button onClick={() => open(r)}
                                                className="w-full flex items-center justify-center gap-2 py-3 bg-orange-500 text-white font-bold rounded-xl text-sm transition-all active:scale-95 shadow-md shadow-orange-100">
